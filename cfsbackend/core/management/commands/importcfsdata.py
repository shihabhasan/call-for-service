...
class Command(BaseCommand):
    ...
    def create_calls(self):
        try:
            ...
            while start < len(self.calls):
                batch = self.calls[start:start + batch_size]
                calls = []
                notes = []
                ...
                for idx, c in batch.iterrows():
                    ...
                    call.month_received = call.time_received.month
                    call.hour_received = call.time_received.hour
                    call.year_received, call.week_received, _ = \
                        call.time_received.isocalendar()
                    call.dow_received = call.time_received.weekday()

                    if call.first_unit_arrive is not None and call.time_received is not None:
                        call.overall_response_time = call.first_unit_arrive - call.time_received

                    if call.first_unit_arrive is not None and call.first_unit_dispatch is not None:
                        call.officer_response_time = call.first_unit_arrive - call.first_unit_dispatch
    ...
    def create_shift_units(self):
        try:
            ...
            while start < len(self.in_service):
                batch = self.in_service[start:start + batch_size]
                shift_units = []
        ...
        while start < len(df):
            ...
            for idx, s in batch.iterrows():
                oos = OutOfServicePeriod(oos_id=safe_int(s.outservid),
                                         ...
                                         shift_id=self.map('Shift',
                                                           s.unitperid))
                if oos.start_time and oos.end_time:
                    oos.duration = oos.end_time - oos.start_time
                ooss.append(oos)
    ...
    def create_call_log(self):
        # TODO find out why shift_id is not being filled normally
        try:
            start = 0
            batch_size = 1000
            while start < len(self.call_log):
                batch = self.call_log[start:start + batch_size]
                cls = []
